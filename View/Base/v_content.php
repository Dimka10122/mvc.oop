...
<nav class="site-nav">
    <div class="container header-nav">
        <ul class="nav right-nav">
            ...
            <?php if ($userInfo->canUser('add_messages')) : ?>
                <li class="nav-item">
            ...
            <?php if (!$userInfo->canUser('create_page', 'control_pages')): ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=BASE_URL?>pages"><?= __('Pages')?></a>
                </li>
            <?php endif;
                if ($userInfo->canUser('statistics')) : ?>
                <li class="nav-item">
        ...
        <ul class="nav left-nav">
            ...
            <?php if ($userInfo->canUser('add_roles', 'edit_roles')): ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=BASE_URL?>roles"><?= __('Role')?></a>
                </li>
            <?php endif;
            if ($userInfo->canUser('block_users')): ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=BASE_URL?>block_users"><?= __('Block')?></a>
                </li>
            <?php endif;
            if ($userInfo->canUser('rise_users')): ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=BASE_URL?>rise/change_rises"><?= __('Rise up requests')?></a>
                </li>
                <?php if ($userInfo->canUser('create_page', 'control_pages')) : ?>
                    <li class="nav-item dropdown show-nav-li">
<!--                        <span class="nav-link dropdown-toggle" data-bind="click: switchMenu" id="navbarDropdown">--><?//=__('Pages')?><!--</span>-->
<!--                        <div class="dropdown-menu" aria-labelledby="navbarDropdown" data-bind="visible: isActiveMenu">-->
<!--                            --><?php //if ($userInfo->canUser('create_page')) : ?>
<!--                                <a class="nav-link dropdown-item" href="--><?//=BASE_URL?><!--pages--><?//=BASE_URL?><!--create_page">-->
<!--                                    --><?//= __('Create')?>
<!--                                </a>-->
<!--                            --><?php //endif;
//                                if ($userInfo->canUser('control_pages')) : ?>
...
<!--                        </div>-->
                ...
                <?php endif;?>
            <?php endif;
            if ($userInfo->canUser('send_rise_request') && $userInfo->userInfoData) : ?>
                <li class="nav-item">
                    <a class="nav-link" href="<?=BASE_URL?>rise/request"><?= __('Ask for a rise')?></a>
                </li>
            <?php endif;
            if ($userInfo->userInfoData) :?>
                <li class="nav-item">
...
<!--<script>-->
<!--    let links = ['Create', 'Control'];-->
<!--    require(['assets/js/scripts/modules/dropdownMenu'], function (dropdown) {-->
<!--        dropdown(links);-->
<!--    });-->
<!--</script>-->
